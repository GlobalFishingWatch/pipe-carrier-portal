...
WITH
  -- SOURCES
  carrier_portal_published_events_encoutners AS (
  SELECT
    *
  FROM
    `proj_carrier_portal_pew.carrier_portal_published_events_encounters_v20200720`),
  named_anchorages AS (
  SELECT
    *
  FROM
  ...
  FROM (
    SELECT
      JSON_EXTRACT_SCALAR(event_vessels,
        "$[0].destination_port.port_id") port_id,
      JSON_EXTRACT_SCALAR(event_vessels,
        "$[0].type") type
    FROM
      carrier_portal_published_events_encoutners
    WHERE
      EXTRACT(year
      FROM
        event_start) > 2017
    ...
    SELECT
      JSON_EXTRACT_SCALAR(event_vessels,
        "$[1].destination_port.port_id") port_id,
      JSON_EXTRACT_SCALAR(event_vessels,
        "$[1].type") type
    FROM
      carrier_portal_published_events_encoutners
